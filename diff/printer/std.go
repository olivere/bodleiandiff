...
func (p *StdPrinter) Print(d diff.Diff) error {
	...
	case diff.Unchanged:
		if p.unchanged {
			fmt.Fprintf(p.w, "Unchanged\t%v\t%v\t%v\n", d.Src.Key, d.Src.ID, cmp.Diff(d.Src, d.Dst))
		}
	case diff.Created:
		if p.created {
			fmt.Fprintf(p.w, "Created\t%v\t%v\t%v\n", d.Dst.Key, d.Dst.ID, cmp.Diff(d.Src, d.Dst))
		}
	case diff.Updated:
		if p.updated {
			fmt.Fprintf(p.w, "Updated\t%v\t%v\t%v\n", d.Src.Key, d.Src.ID, cmp.Diff(d.Src, d.Dst))
		}
	case diff.Deleted:
		if p.deleted {
			fmt.Fprintf(p.w, "Deleted\t%v\t%v\n", d.Src.Key, d.Src.ID)
		}
	}
	return nil
}
